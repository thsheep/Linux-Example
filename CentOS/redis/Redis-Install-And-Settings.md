...
cp /usr/local/redis-3.2.3/utils/redis_init_script /etc/rc.d/init.d/redis  

2.修改redis服务脚本，vi/etc/rc.d/init.d/redis；修改完的/etc/rc.d/redis如下：
...
2.1在文档 #!/bin/sh添加# chkconfig: 2345 80 90
2.2将REDISPORT=6379改成需要的端口号；
2.3注意到EXEC=/usr/local/bin/redis-server,假如/usr/local/bin下没有对应的redis-server、redis-cli等可执行文件，则需要将其拷贝到/usr/local/bin/目录下；
2.4注意到CONF="/etc/Redis/${REDISPORT}.conf"。将redis配置文件拷贝到/etc/redis/下,命令如下： 
[plain] view plain copy
mkdir /etc/redis  
cp /usr/local/redis-3.2.3/redis.conf /etc/redis/6379.conf  

2.5.在start方法中的$EXEC $CONF 后边加一个&，表示让redis作为后台服务启动，否则该服务会独占输入屏。


3.将/etc/rc.d/init.d/redis脚本添加到服务，命令如下：
[plain] view plain copy
chkconfig --add redis  

4.启动redis： service redis start
  停止redis:   service redis stop

5.开机启动： chkconfig redis on  




  Reids性能测试工具：Redis-benchmark